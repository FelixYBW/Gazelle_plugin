...
inline arrow::Status WriteValue(uint8_t* buffer_address, int32_t field_offset,
...
arrow::Status ColumnarToRowConverter::Write() {
  ...
  std::vector<uint16_t> fixcol;
  /*for (auto col_index = 0; col_index < num_cols_; col_index++) {
    auto array = rb_->column(col_index).get();
  ...
  for (auto col_index = 0; col_index < num_cols_; col_index++) {
    ...
    switch (array->type_id()) {
      case arrow::BooleanType::type_id: {
        // Boolean type
        auto bool_array = (arrow::BooleanArray*)(array);
      ...
      case arrow::BinaryType::type_id: {
        // Binary type
        auto binary_array = (arrow::BinaryArray*)(array);
        using offset_type = typename arrow::BinaryType::offset_type;
        const offset_type* offsetptr = binary_array->raw_value_offsets();
        const auto dataptr = binary_array->raw_data();

        if (array->null_count()==0)
        {
          for (auto i = 0; i < num_rows; i++) {
            offset_type length = offsetptr[i+1] - offsetptr[i];
            auto value = &dataptr[offsetptr[i+1]];
      ...
      case arrow::Decimal128Type::type_id: {
        auto out_array = dynamic_cast<arrow::Decimal128Array*>(array);
      ...
      case arrow::ListType::type_id: {
        auto list_array = dynamic_cast<arrow::ListArray*>(array);
        for (auto i = 0; i < num_rows; i++) {
          bool is_null = array->IsNull(i);
          if (is_null) {
            SetNullAt(buffer_address, offsets[i], field_offset, col_index);
  ...
  #define PROCESS(ARRAYTYPE, BYTES) {                                                     \
        ...
        {                                                                 \
          auto numeric_array = dynamic_cast<arrow::ARRAYTYPE*>(array);               \
          for (auto i = 0; i < num_rows; i++) {                          \
            bool is_null = array->IsNull(i);                                  \
            if (is_null) {                                                      \
              SetNullAt(buffer_address, offsets[i], field_offset, col_index);                \
